...
using PlutoUI, LinearAlgebra, SparseArrays, Images, VideoIO
...
> "This paper delivers some rather surprising news: one can disentangle the low-rank and sparse components exactly by convex programming, and this provably works under very broad conditions that are much broader than those provided by the best known results."
...
Let $A=L+S$ be the splitting that we are looking for. Assume 

$$
L=U\Sigma V^*=\sum_{i=1}^r u_i\sigma_iv_i^*.$$
...
that is we seek the best $l^2$ rank-$k$ estimate of $L$.


2. The problem makes sense if the __incoherence conditions__ 

$$
\max_{\displaystyle i} \| U^* e_i\|_2^2\leq \frac{\mu r}{m}, \quad
\max_{\displaystyle i} \| V^* e_i\|_2^2\leq \frac{\mu r}{n}, \quad
...
exactly recovers $L$ and $S$. Here $\lambda$ is a regularization parameter, usually $\lambda=\displaystyle\frac{1}{\sqrt{m}}$ or $\lambda=\displaystyle\frac{1}{\sqrt{mn}}$

4. __Principal Component Pursuit by Alternating Directions__ algorithm finds the above estimate
    1. Initialization: $S=0$, $Y=0$, $L=0$, $λ>0$, $\mu>0$, $\delta=10^{-7}$.
    2. Iterations: __while__ $\|A-L-S\|_F>\delta\|A\|_F$ __repeat__
        1.  $L=\mathcal{D}_{\displaystyle \mu^{-1}}(A-S+\mu^{-1}Y)\qquad$ # Singular value thresholding 
        2.  $S=\mathcal{S}_{\displaystyle \lambda \mu^{-1}}(A-L+\mu^{-1}Y) \qquad$ # Shrinkage
        3.  $Y=Y+\mu(A-L-S)\qquad$ # Updating
"""
...
function PCPAD(A::Array; δ::Real=1.0e-7, maxiter::Int=200)
    # Initialize
    tol=δ*norm(A)
    m,n=size(A)
    S=Y=L=zero(A)
...
md"""
# Surveilance video example

We are using surveillance camera video specially directed to test splitting algorithms. The video is 229.2 seconds long and has 10 frames per second. 
"""

# ╔═╡ 1a00edf8-fe40-44c3-aa37-d83ff095f936
varinfo(VideoIO)

# ╔═╡ 06cf7873-3084-43c2-a80b-a2e60456c4aa
...
# ╔═╡ e3004fde-74f6-4737-a113-2036733c3d19
size(clip[1])

# ╔═╡ 28e3a253-c3ff-44e8-917a-ab9d52b4bd69
begin
	# Turn frames into tall matrix
	mᵢ,nᵢ=size(clip[1])
	mᵥ=mᵢ*nᵢ
	nᵥ=length(clip)
	Aᵥ=Array{Float64}(undef,mᵥ,nᵥ)
	for i in 1:nᵥ
	    Aᵥ[:,i]=vec(float(clip[i]))
	end
end

# ╔═╡ e3fe345e-a432-4efa-8078-6b8b5c752ead
size(Aᵥ), norm(Aᵥ)

# ╔═╡ ec14b074-5649-48e1-8c6a-9c2793d17bd1
# Some timings for orientation
@time qr(Aᵥ);

# ╔═╡ 175f9616-4a1a-4db9-af86-d14ea932ff2b
@time D(Aᵥ,0.5);

# ╔═╡ 0e3af701-8279-48e6-9a77-fcbfc23ed256
# Compute the splitting < 2 minutes
@time Lᵥ,Sᵥ,itersᵥ=PCPAD(Aᵥ);

# ╔═╡ deff3de3-98ee-43ba-8ed6-b18d9bc8d3bc
# Some information. Although the maximal number of iterations has been reached, 
# the results are still great.
itersᵥ, rank(Lᵥ), norm(Aᵥ-Lᵥ-Sᵥ), norm(Aᵥ)

# ╔═╡ 08459a4c-becf-4114-a357-6c599426890a
svdvals(Lᵥ)[1:20]

# ╔═╡ 8eca3e19-48eb-461c-bc3d-e7313a6fe2b0
begin
	# Restore all frames of low-rank and sparse part
	# Notice the normalization of the sparse part by 0.5
	LowRank=similar(clip)
	Sparse=similar(clip)
	for i=1:nᵥ
	    LowRank[i]=reshape(Lᵥ[:,i],mᵢ,nᵢ)
	    Sparse[i]=clamp01.(reshape(Sᵥ[:,i],mᵢ,nᵢ).+0.5)
	end
end

# ╔═╡ 109190eb-6cb6-47f7-9d99-1be63eb6b53f
# Play the original clip, low-rank and sparse parts
@bind t1 Clock(interval=0.1, max_value=50)

# ╔═╡ bbde636e-e726-4057-9419-0403b7dbc69b
[clip[t1] LowRank[t1]  Sparse[t1]]
...
manifest_format = "2.0"
project_hash = "68b589fd427d93107cc0c6f00b4caa27aa0c0ebd"
